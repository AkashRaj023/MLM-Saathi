        ...
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-10">
            {['API Health', 'SLA Performance', 'Revenue Impact', 'AI Actions'].map((item) => (
              <a key={item} href={`#${item.toLowerCase().replace(' ', '-')}`} className="text-[10px] font-black uppercase tracking-widest text-gray-400 hover:text-[#0A1628] transition-colors">{item}</a>
            ))}
          </div>
...
const FeatureItem = ({ icon, title, desc }: { icon: React.ReactNode; title: string; desc: string }) => (
  <div className="flex gap-6 group">
    <div className="shrink-0 w-12 h-12 bg-white/10 rounded-xl flex items-center justify-center text-[#00B4D8] group-hover:bg-[#00B4D8] group-hover:text-[#0A1628] transition-all">
      {icon}
    </div>
    <div>
